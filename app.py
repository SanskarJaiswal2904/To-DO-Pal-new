...
# # def get_notes():
# #     notes = list(db.noteInfoCollection.find({}, {"_id": 1, "title": 1, "description": 1, "datecreated": 1}))
# #     for note in notes:
...
#     # Delete notes with the matching userId
#     result = db.noteInfoCollection.delete_many({"userId": user_object_id})
#     if result.deleted_count > 0:
#         return jsonify({"message": "All notes deleted successfully"}), 200
#     else:
...
# @app.route("/notes/deleteone/<string:sno>", methods=["DELETE"])
# def delete_one_note(sno):
#     note = db.noteInfoCollection.find_one({"_id": sno})
#     if note:
#         db.noteInfoCollection.delete_one({"_id": sno})
#         return jsonify({"message": f"Note with ID {sno} deleted successfully"}), 200
#     else:
#         return jsonify({"error": f"No note found with ID {sno}"}), 404
...
#     title = data.get('title')
#     description = data.get('description')
#     is_completed = data.get('isCompleted')  # Get the isCompleted status from the request

#     # Create the update document dynamically based on the provided fields
#     update_fields = {}
#     if title is not None:
#         update_fields['title'] = title
#     if description is not None:
